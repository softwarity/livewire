...
export class LiveWindowDataSource<Row extends LiveRow> extends DataSource<Row | undefined> {
   ...
   * the screen holds stale rows until something unrelated wakes Angular up. A
   * signal the view reads is how that is said properly - `ApplicationRef.tick()`
   * from the callback is the other way, and it throws when it lands inside a
